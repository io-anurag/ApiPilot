...
function baseStages() {
  ...
  return Object.fromEntries(ids.map((id) => [id, { stageId: id, status: "not-yet-reached" }]));
}
...
describe("TestGenerationWorkflowPage", () => {
  ...
  it("shows the upload prompt when no workflow is in progress (FR-017)", async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText("Upload OpenAPI specification")).toBeInTheDocument());
  });
  ...
  it("walks upload -> apiReview -> deterministicGeneration -> aiEnhancement -> scenarioReview", async () => {
    stubFetch([
      ...
      { workflow: workflowAt("aiEnhancement", { deterministicTestModel: { scenarios: [] } }) },
      {
    ...
    const file = new File(["openapi: 3.0.3"], "valid.yaml", { type: "application/x-yaml" });
    fireEvent.change(screen.getByLabelText("Upload OpenAPI specification"), { target: { files: [file] } });
    await waitFor(() => expect(screen.getByTestId("api-review-stage")).toBeInTheDocument());

    fireEvent.click(screen.getByRole("button", { name: "Continue" }));
    await waitFor(() => expect(screen.getByTestId("deterministic-generation-stage")).toBeInTheDocument());

    fireEvent.click(screen.getByRole("button", { name: "Generate Baseline Test Suite" }));
    await waitFor(() => expect(screen.getByTestId("ai-enhancement-stage")).toBeInTheDocument());

    fireEvent.click(screen.getByRole("button", { name: "Enhance with AI" }));
    await waitFor(() => expect(screen.getByTestId("scenario-review-stage")).toBeInTheDocument());
  });
  ...
  it("shows the workflow-level stage tracker once a workflow starts (User Story 2)", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("workflow-stage-tracker")).toBeInTheDocument());
