...
- [X] T001 [P] Extend `Parameter` in `packages/shared-domain/src/apiModel.ts`: add optional
...
- [X] T002 [P] Extend `GenerationLimitationKind` in `packages/shared-domain/src/postmanArtifact.ts`:
...
- [X] T003 [P] Write failing unit tests for `extractParameters`'s new `style`/`explode`/
...
- [X] T004 Implement the extraction changes in `extractParameters`
...
- [X] T005 [P] Create `backend/tests/fixtures/postman/parameterFixtures.ts`: operations and
...
- [X] T006 [P] Write failing unit tests for `resolveParameterStyle`, `percentEncode`,
...
- [X] T007 Implement `backend/src/postman/parameterSerialization.ts`
...
- [X] T008 [P] [US1] Write failing tests in `backend/tests/unit/postman/requestItem.test.ts`
...
- [X] T009 [P] [US1] Write failing integration test in
...
- [X] T010 [US1] In `buildUrl` (`backend/src/postman/requestItem.ts`), for a query parameter whose
...
- [X] T011 [US1] In the same file's path-segment and header-building code, call
...
- [X] T012 [US1] Run T008/T009 to green (`npm run test -w backend -- tests/unit/postman/requestItem.test.ts tests/integration/postmanCollection.test.ts`); confirm no existing test in
...
- [X] T013 [P] [US2] Write failing tests in `backend/tests/unit/postman/requestItem.test.ts`
...
- [X] T014 [P] [US2] Write failing test in `backend/tests/integration/postmanCollection.test.ts`
...
- [X] T015 [US2] Update the remaining scalar rendering call sites in `buildUrl`
...
- [X] T016 [US2] Run T013/T014 to green. Verify `backend/tests/unit/postman/reexportStability.test.ts`
...
- [X] T017 [P] [US3] Write failing tests in `backend/tests/unit/postman/requestItem.test.ts`
...
- [X] T018 [P] [US3] Write failing test in `backend/tests/integration/postmanCollection.test.ts`
...
- [X] T019 [US3] Add the `unresolved-parameter-style` limitation-recording branch alongside the
...
- [X] T020 [P] [US3] Add a `"unresolved-parameter-style"` entry to `LIMITATION_HEADINGS` in
      `backend/src/postman/readme.ts`.
- [X] T021 [P] [US3] Add a `"unresolved-parameter-style"` entry to `LIMITATION_HEADINGS` in
      `frontend/src/components/PostmanExportLimitations.tsx`.
- [X] T022 [US3] Run T017/T018 to green.
...
- [X] T023 [P] Walk through quickstart.md's 8 validation scenarios and confirm each is covered by
      a passing automated test (cross-reference against T008/T009/T013/T014/T017/T018).
- [X] T024 Run full repository validation from the repo root: `npm run build`, `npm run lint`,
      `npm test`; fix any fallout across workspaces.
- [X] T025 If implementation diverged from plan.md/data-model.md's design during T001-T022 (e.g. a
      ...
      Skip this task if no divergence occurred. — Evaluated: `requestItem.ts` was confirmed as the
      sole production rendering call site (no second call site, unlike spec 021's
      `workflowRendering.ts` surprise); the header-parameter limitation branch is a natural
      extension of `resolveParameterStyle`'s already location-agnostic design, not a deviation.
      No "As built" note needed.
