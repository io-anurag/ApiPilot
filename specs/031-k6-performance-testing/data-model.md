...
| `fingerprint` | `string` | SHA-256 over the canonical JSON of every field above, plus the ids of the scenarios and workflows used. It is the out-of-date check (FR-023). |
...
| `statusCheck` | `{expected: string[]} \| {none: "no-documented-success-status"}` | FR-012. Values like `"201"` or `"2XX"`. |
...
| Field | Type | Rule |
|---|---|---|
| `totals` | `{requests, errors, errorRatePercent, iterations, iterationsCutShort, throughputPerSecond}` | Throughput is requests divided by the actual run duration. |
...
`{stepId, operationKey, method, requests, latencyMs: {p50, p90, p95, p99}, throughputPerSecond,
errorRatePercent, errorsByStatus: {status: string, count}[], errorsByCategory: {category:
FailureCategory, count}[], checkPassRatePercent, notAttempted: {missingData: number,
dependencyNotAttempted: number}, missingVariables: string[]}`.

`FailureCategory = "unexpected-status" | "connection-error" | "timeout" | "extraction-failed" |
"missing-data" | "dependency-not-attempted" | "authentication" | "rate-limited"` (D14).

### `PerformanceFinding`

`{ruleId: "threshold-failed" | "slowest-step" | "failures-start" | "cut-short-iterations" |
