...
- [X] T001 Run `npm test -w frontend`, `npm run lint -w frontend`, and `npm run build -w frontend`
  from the repo root; record the current pass/fail counts as the baseline that Phase 6 (Polish)
  will diff against.

  **Baseline recorded 2026-09-20**: `npm run lint` — clean. `npm run build -w frontend` — clean.
  `npm test -w frontend` — **209 passed, 1 failed, 210 total** (36/37 files). The one failure,
  `frontend/tests/unit/App.test.tsx` ("Import a Postman Collection" text not found after
  switching tabs), is a **pre-existing bug on `main`**, unrelated to this feature: a prior commit
  (`cec78b4`, "enhance external collection upload and list components") restyled
  `ExternalCollectionUpload.tsx` and dropped that heading text without updating this assertion.
  This is not something this feature introduced or is in scope to fix (out of scope per FR-014);
  Phase 6's regression diff (T055) treats 209/210 as the baseline to match, not 210/210, and this
  pre-existing failure is reported separately from this feature's own results.
...
- [X] T002 [P] [US1] Create `frontend/src/components/workflowStageViewModel.ts`: move
...
- [X] T003 [US1] Update `frontend/src/components/WorkflowStageTracker.tsx` to import
  ...
  T002.)
- [X] T004 [P] [US1] Create `frontend/src/components/AppHeader.tsx`: extract the `<header>...
  ...
  (FR-001).
- [X] T005 [US1] Update `frontend/src/App.tsx` to render `<AppHeader health={health} />` in place
...
- [X] T006 [P] [US1] Extend `frontend/tests/unit/WorkflowStageTracker.test.tsx` with a case
...
- [X] T007 [P] [US1] Create `frontend/tests/unit/AppHeader.test.tsx` covering the three
...
- [X] T008 [US1] Update `frontend/tests/unit/App.test.tsx` so its header-related assertions target
...
- [X] T009 [P] [US2] Create `frontend/src/components/EmptyState.tsx`: shared "no data" presentation
  (message, optional secondary text) per FR-006, matching this repo's single-file-per-concern
  convention (cf. `StatusBadge.tsx`).
- [X] T010 [P] [US2] Create `frontend/src/components/ErrorState.tsx`: shared error presentation
  ...
  per FR-006/FR-007.
- [X] T011 [P] [US2] Create `frontend/src/components/Skeleton.tsx`: shared loading-placeholder
...
- [X] T012 [P] [US2] Create `frontend/src/components/Dialog.tsx`: extract the generic modal shell
...
- [X] T013 [P] [US2] Create `frontend/src/components/Tabs.tsx`: a shared tabbed-panel primitive
...
- [X] T014 [US2] Update `frontend/src/App.tsx` to render its two top-level views ("Guided
  ...
  (Depends on T013.)
- [X] T015 [US2] Refactor `frontend/src/components/ConfirmDialog.tsx` to compose `Dialog.tsx`
...
- [X] T016 [P] [US2] Migrate the inline error message (~line 89) in
...
- [X] T017 [P] [US2] Migrate the two inline error messages (~lines 481, 521) in
...
- [X] T018 [P] [US2] Migrate the inline error message (~line 25) in
  `frontend/src/components/AnalysisSummary.tsx` to `ErrorState`. (Depends on T010.)

  **Not applicable, discovered during implementation**: the `role="alert"` block at this line is
  `AnalysisSummary`'s collapsible specification-issues disclosure (warning tone, mirroring
  `WorkflowStageTracker`'s own issues banner) — not an error condition FR-007 targets. No ad hoc
  error message exists in this file. Left unchanged.
- [X] T019 [P] [US2] Migrate the inline error message (~line 184) in
...
- [X] T020 [P] [US2] Migrate the three inline error messages (~lines 427, 616, 752) in
...
- [X] T021 [P] [US2] Migrate the inline error message (~line 587) in
...
- [X] T022 [P] [US2] Migrate the inline error message (~line 167) in
...
- [X] T023 [P] [US2] Migrate the inline error message (~line 380) in
  ...
  (Depends on T010.)
- [X] T024 [P] [US2] Migrate the inline error message (~line 212) in
...
- [X] T025 [P] [US2] Migrate the inline error message (~line 157) in
...
- [X] T026 [P] [US2] Migrate the inline error message (~line 311) in
...
- [X] T027 [P] [US2] Migrate the inline error message (~line 72) in
...
- [X] T028 [P] [US2] Migrate the two inline error messages (~lines 68, 102) in
...
- [X] T029 [P] [US2] Migrate the two inline error messages (~lines 211, 395) in
...
- [X] T030 [P] [US2] Migrate the "No uploaded collections yet" message (~line 46) in
  ...
  (Depends on T009.)
- [X] T031 [P] [US2] Migrate the "No results match the current filter" message (~line 310) in
...
- [X] T032 [P] [US2] Migrate the "no accepted scenarios to export" message (~line 205) in
...
- [X] T033 [P] [US2] Migrate the empty message (~line 31) in
...
- [X] T034 [P] [US2] Migrate the "No scenarios match the current filters" message (~line 238) in
...
- [X] T035 [US2] Replace the ad hoc pulsing-dot "Connecting…" indicator in
  ...
  **Extended during implementation**: two more genuine content-loading indicators (not button
  states — no button involved) were found and migrated onto `Skeleton` for the same consistency
  reason: `ScenarioReviewStage.tsx`'s "Running dependency analysis…" in-progress notice, and
  `TestGenerationWorkflowPage.tsx`'s "Restoring workflow…" indicator (which already used a manual
  `animate-pulse` dot — now the shared primitive instead of a one-off copy).
- [X] T036 [P] [US2] Add `frontend/tests/unit/EmptyState.test.tsx`. (Depends on T009.)
- [X] T037 [P] [US2] Add `frontend/tests/unit/ErrorState.test.tsx`. (Depends on T010.)
- [X] T038 [P] [US2] Add `frontend/tests/unit/Skeleton.test.tsx`, including a reduced-motion
  assertion. (Depends on T011.)
- [X] T039 [P] [US2] Add `frontend/tests/unit/Dialog.test.tsx` covering focus trap, Escape-to-close,
  and initial focus. (Depends on T012.)
- [X] T040 [P] [US2] Add `frontend/tests/unit/Tabs.test.tsx` covering keyboard navigation and
...
- [X] T041 [US3] Add dark-mode color token overrides to `frontend/src/index.css`'s `@theme` block
...
- [X] T042 [P] [US3] Add `dark:` variants for any raw (non-token) color classes in
  `frontend/src/components/controlStyles.ts` (`BUTTON_STYLES`). (Depends on T041.)
- [X] T043 [P] [US3] Add `dark:` variants for any raw color classes in
  `frontend/src/components/StatusBadge.tsx`. (Depends on T041.)
- [X] T044 [P] [US3] Add `dark:` variants for any raw color classes in
  `frontend/src/components/HttpMethodBadge.tsx`. (Depends on T041.)
- [X] T045 [P] [US3] Add `dark:` variants for any raw color classes in
  `frontend/src/components/ProvenanceBadge.tsx`. (Depends on T041.)
- [X] T046 [P] [US3] Add `dark:` variants for any raw color classes in
  `frontend/src/components/WorkflowStageTracker.tsx`. (Depends on T041, T003.)
- [X] T047 [P] [US3] Add `dark:` variants for any raw color classes in
  `frontend/src/components/AppHeader.tsx`. (Depends on T041, T035.)

  **No change needed, verified**: `AppHeader` is intentionally dark-styled regardless of the
  system theme (fixed `bg-slate-950`, a deliberate always-dark top bar, like many dev tools'
  chrome) — every color inside it was already chosen against that fixed dark background. Its one
  token-based color (`VersionBadge`'s `text-muted`) also verified fine: the dark-mode `--color-
  muted` redefinition (T041) makes it lighter, which only improves its contrast against this
  already-dark bar.
- [X] T048 [P] [US3] Add `dark:` variants for any raw color classes in
  `frontend/src/components/EmptyState.tsx`, `ErrorState.tsx`, `Skeleton.tsx`, `Dialog.tsx`, and
  `Tabs.tsx`. (Depends on T041, T009-T013.)

  **Also fixed while touching `ErrorState.tsx`**: its `border-danger-200` referenced a shade this
  theme never defines (danger only has 50/100/500/600/700) — the same pre-existing, undefined-
  shade pattern found (but *not* fixed, as out of scope) at nine other call sites across the app;
  see the T052 note below.
- [X] T049 [US3] Verify and, if needed, adjust responsive/truncation behavior (FR-009) of
  ...
  **Found and fixed**: `Tabs.tsx` had no overflow handling at all — added `overflow-x-auto`
  (matching `WorkflowStageTracker`'s own convention) so two tab labels can't force horizontal
  page scroll on a narrow viewport. `AppHeader`/`WorkflowStageTracker` were already correct
  (`truncate`/`min-w-0`, horizontal-scrolling stage strip); neither carries a dynamic long value
  in the first place — that lives in other, not-yet-restyled pages, deferred per FR-014.
  `EmptyState`/`ErrorState`/`Skeleton`/`Dialog` have no fixed widths or `nowrap`, so text wraps
  naturally; `Dialog`'s backdrop `p-4` keeps its `max-w-md` panel off the viewport edges.
- [X] T050 [US3] Audit focus-visible styling (FR-011) across every new/extended interactive
  element from US1/US2, applying the existing `focus-visible:ring-2 focus-visible:ring-brand-500`
  convention wherever missing.

  **No changes needed, verified**: `Tabs.tsx` kept the convention through extraction;
  `WorkflowStageTracker`'s buttons and every `ConfirmDialog`/`Dialog` button already had it from
  before this feature; `EmptyState`/`ErrorState`/`Skeleton`/`AppHeader` contain no interactive
  elements.
- [X] T051 [US3] Make `WorkflowStageTracker.tsx`'s `scrollIntoView({ behavior: "smooth" })` call
...
- [X] T052 [US3] Audit text/background contrast (SC-004) for every `StatusBadge` tone,
  ...
  **Computed (WCAG relative-luminance formula) for every tone pair**: all pass AA (≥4.5:1 for
  normal text). Light mode (existing, unchanged, bg-100/text-700): neutral 9.45:1, info 4.79:1,
  success 4.57:1, warning 4.51:1, danger 5.30:1, brand/PATCH 4.99:1 — **warning's 4.51:1 clears
  the 4.5:1 bar by a hair**; not a failure, but flagged as a thin pre-existing margin worth
  hardening in a future contrast pass rather than something this phase needed to change. Dark
  mode (new, `dark:bg-{tone}-500/15` composited over `#0d1211` / `dark:text-{tone}-100`): all
  11.9–14.9:1, comfortably exceeding AA. Also found, while computing this: `border-{tone}-200` is
  referenced at 8 pre-existing call sites across 7 files (`AiEnhancementStage.tsx` ×2,
  `AnalysisSummary.tsx`, `PostmanExportPanel.tsx`, `PostmanExportLimitations.tsx`,
  `PostmanGenerationStage.tsx`, `ScenarioReviewStage.tsx`, `AiEnhancementOutcomeSummary.tsx`) for
  a shade this theme never defines (success/warning/danger/info only have 50/100/500/600/700) — Tailwind silently drops
  the rule, leaving a default border color instead of the intended tone. This pre-dates this
  feature and sits entirely in page content this phase doesn't touch (FR-014), so it was **not**
  fixed here (only in the one new file that would otherwise have copied the same mistake,
  `ErrorState.tsx` — see the T048 note); it's a real, if minor, defect worth its own follow-up
  fix.
- [X] T053 [P] [US3] Extend `frontend/tests/unit/TestGenerationWorkflowAccessibility.test.tsx` and
...
- [X] T054 [US3] Extend `frontend/tests/unit/AppHeader.test.tsx` with a narrow-width/truncation
...
- [X] T055 [P] Re-run `npm test -w frontend`, `npm run lint -w frontend`, and
  `npm run build -w frontend`; diff results against the T001 baseline and confirm zero regressions
  (SC-003).

  **Result**: lint clean; build clean (65→72 modules, +7 new files; CSS +4.1KB/+0.47KB gzip, JS
  +1.5KB/+0.8KB gzip — negligible, zero new dependencies); tests **233 passed, 1 failed, 234
  total** (42/43 files) vs. the T001 baseline of 209/1/210 (36/37) — **+24 new tests, zero
  regressions**, same single pre-existing failure carried forward unchanged.
- [X] T056 Execute quickstart.md's four manual validation scenarios in a real browser, including
  the External Collections regression check.

  **Verified with a real headless-Chromium session** (backend + frontend dev servers, Playwright,
  zero console errors throughout): (1) uploaded `backend/tests/fixtures/openapi/valid.yaml` and
  confirmed `WorkflowStageTracker` renders live stage statuses and the new locked-stage reason
  text exactly as designed — "Not yet reached — Complete API Review first" on "Deterministic
  Generation" while "API Review" was active; (2) confirmed the shell, `Tabs`, badges, and the new
  lock-reason text are fully legible in both light and dark OS color-scheme, and confirmed — as
  the spec's own out-of-scope boundary (FR-014) predicts — that page *content* below the shell
  (the operations table) is **not** dark-mode-ready yet, a real, expected, correctly-deferred gap
  rather than a silent one; (3) confirmed no horizontal overflow at a 380px viewport in either
  theme; (4) switched to "Import & Run Collection", confirmed it still works unchanged, and saw
  the migrated `EmptyState` ("No uploaded collections yet") rendering correctly. Tab-key focus
  rings were clearly visible in dark mode. Screenshots retained in the session scratchpad.
- [X] T057 Audit the full app for any remaining ad hoc header, navigation, tab, badge, loading,
  ...
  **Result**: a final `role="alert"`/"No …" grep across `frontend/src` turned up nothing new —
  every remaining hit is either the shared components themselves, `AppHeader`'s connection-status
  pill (a compact live-status badge, not an `ErrorState`-shaped condition — correctly out of
  scope), or `ScenarioReviewStage`'s bulk-decision-outcome summary (a structured per-item results
  report, the same distinct concept as `BatchOutcomeList`/`AiEnhancementOutcomeSummary` — not a
  simple error message, and explicitly page content deferred by FR-014). No real gap found; no
  follow-up task added. SC-002 ("100% of pages use the shared shell/components") holds for
  everything this phase is scoped to touch: the shell and the five shared primitives.
