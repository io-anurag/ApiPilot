...
operations / 1,350 deterministic scenarios. At the measured 30-60s per operation (superseding an
earlier ~21s estimate — see Complexity Tracking), whole-specification enhancement is practical to
roughly 5–10 operations at the default run budget.
...
|---|---|---|
| **XXXII (review practical at real scale)** — measured against a real 39-operation specification at 30-60s/operation (superseding an initial ~21s/operation, 15–30-operation estimate; see research.md Decision 5 addendum, Phase 8/T063), whole-specification enhancement is impractical beyond roughly 5–10 operations at the default budget. A 200-operation specification would need well over an hour for full coverage. | This feature makes AI enhancement work *at all*, where it currently produces nothing at any size. The run ceiling converts an impossible run into a useful partial one. | Larger units were measured and fail — 2 and 3 operations both truncate. A faster model is out of scope and slower on CPU. The real answer is **letting the user choose which operations to enhance**, already identified as the fast follow-up; this feature is a prerequisite for it, since selection is only useful once a per-operation request reliably succeeds. |
| **`DEFAULT_ENHANCEMENT_OPERATIONS_PER_UNIT` drifted to `2` without measurement** — a later, unrelated change (retry-logic work, commit `9b93bdc`) raised this constant from Decision 1's measured `1` to `2` "so normal specifications are processed in fewer expensive model calls," with no new measurement backing it and no update to research.md. | Decision 1 directly measured that 2 operations truncate mid-document; shipping that default risks silently reintroducing the failure mode this whole feature exists to fix, for any deployment that leaves the variable unset. | Not "leave it as a considered trade-off" — there was no trade-off analysis behind the `2`, only an unverified assumption about throughput. Corrected the default back to `1` in `backend/src/ai/modelConfig.ts` and `backend/src/ai/localProvider.ts` (Phase 8/T063); the repository's own `.env` already overrode it to `1`, so no currently-running instance changed behavior — only the undocumented fallback did. |
| **`DEFAULT_DECODE_MS_PER_TOKEN` changed from `130` to `180` with no accompanying comment** (commit `7e0621a`) | Decision 8 records `130` against a measured ~140 ms/token. The later `180` is undocumented but biases the pre-flight estimate toward *refusing* more marginal runs, not admitting hopeless ones — the fail-safe direction (XIX) — so it was left as-is rather than reverted on a guess. | Reverting without a real remeasurement would itself be an unverified change in the other direction. T061 (Phase 8) re-measures this against real hardware; until then the current value is documented rather than silently trusted or silently changed. |
