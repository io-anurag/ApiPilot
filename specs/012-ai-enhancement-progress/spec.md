...
- **FR-005**: For a specification whose AI enhancement completes in a single batch, the user-visible experience (total time to result, information shown) MUST be unchanged from today's behavior. This requirement is superseded by `013-ai-enhancement-viability`: the former context-window defect made single-batch execution the only reachable case, so hiding progress for single-batch runs suppressed progress for effectively all real runs. The viability feature makes single-batch progress visible while preserving the existing final outcome contract.
