...
- [ ] T001 Create `frontend/src/logger.ts`: `LogLevel` (`"info"|"warn"|"error"`) and `LogFields`
  ...
  FR-011's clock half).
- [ ] T002 Create `frontend/tests/unit/logger.test.ts` (depends on T001): assert (a) each level
...
- [ ] T003 [P] [US1] Add a test in `frontend/tests/unit/executionClient.test.ts` (new file)
...
- [ ] T004 [P] [US1] Add a test in `frontend/tests/unit/healthClient.test.ts` (new file) — same
  assertion as T003, for `frontend/src/services/healthClient.ts`'s one `catch` block (FR-010).
- [ ] T005 [P] [US1] Extend `frontend/tests/unit/postmanCollectionsClient.test.ts` (existing file)
  with the same logging assertion for `frontend/src/services/postmanCollectionsClient.ts`'s
  `catch` block (FR-010).
- [ ] T006 [P] [US1] Extend `frontend/tests/unit/reviewsClient.test.ts` (existing file) with the
  same logging assertion for `frontend/src/services/reviewsClient.ts`'s `catch` block (FR-010).
- [ ] T007 [P] [US1] Add a test in `frontend/tests/unit/specificationsClient.test.ts` (new file) —
  same assertion, for `frontend/src/services/specificationsClient.ts`'s `catch` block (FR-010).
- [ ] T008 [P] [US1] Add a test in `frontend/tests/unit/testGenerationWorkflowClient.test.ts` (new
  file) — same assertion, for every `catch` block in
  `frontend/src/services/testGenerationWorkflowClient.ts` (FR-010).
- [ ] T009 [P] [US1] Add a test in `frontend/tests/unit/testModelsClient.test.ts` (new file) —
  same assertion, for `frontend/src/services/testModelsClient.ts`'s `catch` block (FR-010).
- [ ] T010 [P] [US1] Create `frontend/tests/unit/globalErrorHandlers.test.ts`: dispatch a synthetic
...
- [ ] T011 [P] [US1] In `frontend/src/services/executionClient.ts`, route every existing `catch`
...
- [ ] T012 [P] [US1] Same retrofit for `frontend/src/services/healthClient.ts` using
  `createLogger("healthClient")` (FR-010; makes T004 pass).
- [ ] T013 [P] [US1] Same retrofit for `frontend/src/services/postmanCollectionsClient.ts` using
  `createLogger("postmanCollectionsClient")` (FR-010; makes T005 pass).
- [ ] T014 [P] [US1] Same retrofit for `frontend/src/services/reviewsClient.ts` using
  `createLogger("reviewsClient")` (FR-010; makes T006 pass).
- [ ] T015 [P] [US1] Same retrofit for `frontend/src/services/specificationsClient.ts` using
  `createLogger("specificationsClient")` (FR-010; makes T007 pass).
- [ ] T016 [P] [US1] Same retrofit for `frontend/src/services/testGenerationWorkflowClient.ts`
  using `createLogger("testGenerationWorkflowClient")` (FR-010; makes T008 pass).
- [ ] T017 [P] [US1] Same retrofit for `frontend/src/services/testModelsClient.ts` using
  `createLogger("testModelsClient")` (FR-010; makes T009 pass).
- [ ] T018 [P] [US1] Create `frontend/src/globalErrorHandlers.ts` exporting
...
- [ ] T019 [US1] Call `installGlobalErrorHandlers()` once from `frontend/src/main.tsx`, before
...
- [ ] T020 [P] [US2] Extend `frontend/tests/unit/logger.test.ts` (depends on T001/T002): stub
...
- [ ] T021 [P] [US2] Create `backend/tests/integration/clientLogs.test.ts` (Supertest against
...
- [ ] T022 [US2] Extend `frontend/src/logger.ts` (Phase 2 module) so `warn`/`error` calls also POST
...
- [ ] T023 [US2] Create `backend/src/api/clientLogs.ts`: a thin router (matching
...
- [ ] T024 [US2] In `backend/src/app.ts`, mount `app.use("/api/client-logs", express.json({ limit:
...
- [ ] T025 [US3] Extend `backend/tests/integration/clientLogs.test.ts` (depends on T021/T023):
...
- [ ] T026 [P] Run `npm test` (both workspaces), `npm run lint`, and `npm run build` from the
  repository root; all must pass with the changes from every phase above.
- [ ] T027 [P] Diff `frontend/package.json`, `backend/package.json`, and `package-lock.json`
  against the pre-feature state and confirm zero new dependencies were added (FR-009, SC-004).
- [ ] T028 Walk through `quickstart.md`'s four manual validation scenarios end-to-end against a
  locally running `npm run dev` instance.
