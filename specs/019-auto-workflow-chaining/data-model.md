...
```ts
/** One relationship promoted to an automatic chain in this export. */
export interface AutomaticChain {
   ...
   *  format `auto_<relationshipId>`, collision-free against real IntegrationWorkflow ids and
   *  against bare pathParameterVariable names. */
  chainId: string;
  relationshipId: string;
  ...
  consumer: { operationPath: string; operationMethod: string; field: string; scenarioId: string };
  variableName: string;
}
...
| Only `confidence` CONFIRMED/LIKELY relationships considered. | FR-003, FR-004 |
| Consumer's field must be an unresolved path parameter of a standalone scenario. | FR-001 |
| Producer operation's positive-outcome scenario must be present in `standaloneResolved` (or already an approved-workflow item — out of scope here since D3 restricts to standalone) for the relevant field. | FR-005, FR-017 |
...
| Producer's item must sort before the consumer's item under the existing `compareRequestSortKeys`. | FR-015 |
| Relationship must be direct (single-hop) — `planAutomaticChains` never chains through an intermediate relationship. | FR-018 |
| Multiple consumers of the same producer field share one `chainId`/variable/extraction. | FR-009 |

## Reused, unmodified types (no data-model change — listed for traceability)

- `ApiDependencyGraph`, `ApiDependencyRelationship`, `DependencyConfidence`, `DeterministicDependencyEvidence`, `AIDependencyCorroboration`, `DependencyCycleFinding`, `IntegrationWorkflow`, `WorkflowVariable` — `packages/shared-domain/src/apiDependency.ts`.
- `WorkflowReviewDecision`, `WorkflowReviewState` — `packages/shared-domain/src/testGenerationWorkflow.ts`.
- `WorkflowExtraction` — `backend/src/postman/assertionScripts.ts`.
- `resolveProducerDisambiguation` — `backend/src/dependencies/mergeRelationships.ts`.
- `applyWorkflowSubstitutions`, `workflowVariableName` — `backend/src/postman/workflowVariables.ts` / `workflowRendering.ts`.
- `compareRequestSortKeys` — `backend/src/postman/ordering.ts`.
