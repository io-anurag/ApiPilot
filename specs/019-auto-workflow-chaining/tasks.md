...
- [ ] T001 [P] Create `backend/src/postman/automaticChaining.ts` with the type-only skeleton from
...
- [ ] T002 [P] Create `backend/tests/fixtures/postman/dependencyFixtures.ts` with builder functions
...
- [ ] T003 Extend `packages/shared-domain/src/postmanArtifact.ts` per data-model.md: add
...
- [ ] T004 Set `provenance.origin: "approved-workflow"` explicitly at the three existing
...
- [ ] T005 Update `backend/src/testGenerationWorkflow/postmanGenerationStage.ts` to build
...
- [ ] T006 Extend the request validators in `backend/src/api/postmanCollections.ts` (e.g.
...
- [ ] T007 Wire the insertion point in `backend/src/postman/generateCollection.ts`: immediately
...
- [ ] T008 [P] [US1] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a single
...
- [ ] T009 [P] [US1] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a single
...
- [ ] T010 [P] [US1] Integration test in `backend/tests/integration/postmanWorkflowCollection.test.ts`:
...
- [ ] T011 [US1] In `backend/src/postman/automaticChaining.ts`, implement relationship lookup for
      ...
      (FR-001, FR-003, FR-004, FR-017). (Depends on T001, T002.)
- [ ] T012 [US1] In `backend/src/postman/automaticChaining.ts`, group eligible relationships by
...
- [ ] T013 [US1] In `backend/src/postman/generateCollection.ts`, apply `planAutomaticChains`'s
...
- [ ] T014 [US1] In `backend/src/postman/generateCollection.ts`, set
      `provenance: { workflowId: chainId, relationshipId, origin: "automatic-chain" }` on every
      `ArtifactVariable` produced by an automatic chain. (Depends on T013.)
...
- [ ] T015 [P] [US2] Unit test in `backend/tests/unit/postman/readme.test.ts`: an export containing
...
- [ ] T016 [P] [US2] Unit test in `backend/tests/unit/postman/generateCollection.test.ts`: the same
      ...
      chains, standard `unresolved-path-parameter` limitations.

### Implementation for User Story 2

- [ ] T017 [US2] Implement the `options.disableAutomaticChaining` short-circuit at the T007 call
...
- [ ] T018 [US2] Extend `backend/src/postman/readme.ts` to render each automatic chain (producer,
...
- [ ] T019 [US3] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a path
      parameter whose only candidate relationship is POSSIBLE confidence is never chained (FR-004).
- [ ] T020 [US3] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: two or more
...
- [ ] T021 [US3] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a relationship
      appearing in `cycles[].relationshipIds` is excluded from chaining (FR-007).
- [ ] T022 [US3] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a producer
...
- [ ] T023 [US3] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a relationship
...
- [ ] T024 [US3] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a relationship
...
- [ ] T025 [US3] Unit test in `backend/tests/unit/postman/automaticChaining.test.ts`: a two-hop
...
- [ ] T026 [US3] In `backend/src/postman/automaticChaining.ts`, exclude any relationship whose id
      appears in `input.cycles[].relationshipIds`. (Depends on T012.)
- [ ] T027 [US3] In `backend/src/postman/automaticChaining.ts`, derive `rejectedRelationshipIds`
...
- [ ] T028 [US3] In `backend/src/postman/automaticChaining.ts`, call the existing
...
- [ ] T029 [US3] At the T007/T013 call site in `backend/src/postman/generateCollection.ts`, apply
      the ordering guard: only keep a chain when the producer's item sorts before the consumer's
      item under the existing, unmodified `compareRequestSortKeys` (`backend/src/postman/ordering.ts`);
      otherwise leave the parameter unresolved. (Depends on T013.)
- [ ] T030 [US3] In `backend/src/postman/automaticChaining.ts`, ensure the relationship lookup never
...
- [ ] T031 [P] Extend `backend/tests/unit/postman/determinism.test.ts` and
      ...
      serialized output across repeated exports (SC-004).
- [ ] T032 Run `npm run test -w backend` (full backend suite) and fix any regression in existing
      007/016 standalone or approved-workflow export tests.
- [ ] T033 [P] Run `npm run lint` and `npm run build` from the repository root and resolve any
      TypeScript/ESLint issue introduced by the new fields or module.
- [ ] T034 Execute quickstart.md's "HTTP Contract Check" and "Guided Workflow Check" manually
