...
| AP-031 — AI Failure Analysis *(post-MVP, formerly AP-018)* (`specs/030-ai-failure-analysis`) | Tasks generated 2026-09-23 (61 tasks in `tasks.md`; implementation not started). On-demand analysis of one failed request in an AP-026 run, persisted through AP-025. Specification context is attached by exact Postman item-id match to the current guided workflow. AP-017's API-only runs are out of scope |
