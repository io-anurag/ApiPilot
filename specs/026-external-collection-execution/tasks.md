...
- [ ] T005 Add `uploaded_collections` and `uploaded_collection_runs` table definitions (data-model.md SQL, via the existing `ensureColumn`-style migration guard) to `backend/src/persistence/connection.ts`.
- [ ] T006 [P] Create `backend/src/externalCollections/errors.ts` with `UploadedCollectionNotFoundError`, `DuplicateNameError`, `InvalidCollectionError`, `InvalidEnvironmentError`, `MissingVariableValuesError`, `RunNotFoundError`, `NoRunInProgressError` — mirroring the shape/naming convention in `backend/src/execution/errors.ts`.
...
- [ ] T012 [P] Unit tests for `uploadedCollectionRepository.ts` in `backend/tests/unit/persistence/uploadedCollectionRepository.test.ts` (mirrors `environmentRepository.test.ts`'s structure): round-trip create/get, `variableValues` stored encrypted (raw DB row does not contain the plaintext value), list ordering, remove.
...
- [ ] T014 [P] Unit tests for `uploadedCollectionStore.ts` in `backend/tests/unit/externalCollections/uploadedCollectionStore.test.ts`: create/list/get/remove, duplicate name within a session refused, same name allowed across two different sessions, not-found on `get`/`remove` of an unknown id.
...
- [ ] T018 [P] [US1] Integration test in `backend/tests/integration/externalCollections/uploadAndRun.test.ts` covering quickstart.md Scenario 1 end-to-end (upload → confirm → start → poll to completion) against the existing `backend/tests/fixtures/execution/targetServer.ts` fixture.
- [ ] T019 [P] [US1] Integration test in `backend/tests/integration/externalCollections/missingVariableValues.test.ts` covering quickstart.md Scenario 2 (missing variable refuses the run start with `400 missing_variable_values`, zero requests dispatched — assert against the target server's received-request count).
- [ ] T020 [P] [US1] Integration test in `backend/tests/integration/externalCollections/malformedUpload.test.ts` covering quickstart.md Scenario 3 (invalid collection JSON → `400 invalid_collection`, not listed afterward; invalid environment JSON → `400 invalid_environment`).
...
- [ ] T023 [P] [US1] Unit tests for `mapUploadedResult.ts` in `backend/tests/unit/externalCollections/mapUploadedResult.test.ts`: passing/failing/mixed test outcomes, no-test-script request (outcome derived from status only, empty `testOutcomes`), connectivity failure, timeout, `rawCapture` present only when `captureRawDetails` is true.
- [ ] T024 [US1] Create `backend/src/api/externalCollections.ts`: `POST /` (multipart upload via the existing `upload` middleware from `backend/src/uploadMiddleware.ts`, `MAX_UPLOAD_BYTES` reused per FR-012), `GET /`, `DELETE /:id`, `POST /:id/execution/start` (FR-004/FR-013 gates only at this point — FR-007 gate added in Phase 4), `POST /:id/execution/cancel`, `GET /:id/execution/runs`, `GET /:id/execution/runs/:runId` — response shapes exactly per `contracts/external-collections-api.md`.
- [ ] T025 [US1] Mount the new router in `backend/src/app.ts`: `app.use("/api/external-collections", upload.fields([...]), externalCollectionsRouter)` following the existing `postmanCollectionsRouter`/multer-mounting pattern already in the file.
- [ ] T026 [P] [US1] Integration test in `backend/tests/integration/externalCollections/listAndRemove.test.ts`: upload, list shows it (without `variableValues`/raw collection body), duplicate name refused with `409 duplicate_name`, remove returns `204`, subsequent list no longer shows it, oversized file refused with `413 file_too_large`.
- [ ] T027 [US1] Create `frontend/src/services/externalCollectionsClient.ts`: thin client for every route in `contracts/external-collections-api.md`, following the existing `fetch`/error-mapping conventions in `frontend/src/services/executionClient.ts`.
- [ ] T028 [US1] Create `frontend/src/components/ExternalCollectionUpload.tsx` (name, tier select, collection file input, environment file input, submit) using Tailwind per CLAUDE.md §26-34 and existing form patterns.
- [ ] T029 [US1] Create `frontend/src/components/ExternalCollectionList.tsx` (list, select, remove) reusing `StatusBadge`/`HttpMethodBadge` conventions.
- [ ] T030 [US1] Create `frontend/src/components/ExternalCollectionRunPanel.tsx` reusing `ExecutionResultsPanel.tsx`'s `RunOverview`/`ExecutionResultList` sub-components where the shape matches (research.md D8/D9), rendering `UploadedRequestResult.testOutcomes` in place of `assertionOutcomes`.
- [ ] T031 [P] [US1] Frontend RTL tests: `frontend/tests/unit/ExternalCollectionUpload.test.tsx`, `frontend/tests/unit/ExternalCollectionList.test.tsx`, `frontend/tests/unit/ExternalCollectionRunPanel.test.tsx` — cover upload form submission, list/select/remove interaction, and run-results rendering (passed/failed/not-attempted rows, testOutcomes detail).

**Checkpoint**: US1 fully functional end-to-end (upload → run → results) and independently testable per spec.md's own Independent Test. FR-007/FR-013 confirmation gates are not yet wired (Phase 4/5) — `execution/start` in this phase runs unconditionally once variables are complete, which is acceptable as an internal checkpoint but MUST NOT be shipped past Phase 4/5.
...
- [ ] T033 [US2] Add `confirmedAt` handling to `uploadedCollectionStore.ts` (T013): a helper to check/set it, called from the execution-start path.
- [ ] T034 [US2] Wire both confirmation gates into `POST /:id/execution/start` in `backend/src/api/externalCollections.ts` (T024) in the order research.md D10 specifies: gate 1 (FR-007, only while `confirmedAt` is unset — sets it permanently on `confirmed: true`) then gate 2 (FR-013, using T009's `findDestructiveRequests()`, every run start) — reusing the exact `confirmation_required` response shape `contracts/execution-api.md` already defines for gate 2.
- [ ] T035 [US2] Add the FR-007 confirmation dialog/step to `frontend/src/components/ExternalCollectionRunPanel.tsx` (T030): distinct copy stating the collection's requests/scripts were not generated or verified by ApiPilot, shown before the first run of a not-yet-confirmed upload; declining/dismissing performs no request (client-side no-op, no partial state).
- [ ] T036 [P] [US2] Frontend RTL test addition in `frontend/tests/unit/ExternalCollectionRunPanel.test.tsx` (T031): confirmation dialog appears for an unconfirmed collection, declining dispatches no start request, accepting proceeds.
...
- [ ] T038 [P] [US3] Integration test in `backend/tests/integration/externalCollections/removalPreservesHistory.test.ts` covering quickstart.md Scenario 6 (FR-017: delete the `UploadedCollectionSet`, past run detail still returns its own `uploadedCollectionSnapshot` unaffected).

### Implementation for User Story 3

- [ ] T039 [US3] Add the FR-015 cross-check (research.md D7) to `backend/src/api/testGenerationWorkflow.ts`'s existing `execution/start` handler: check `uploadedCollectionExecutionStore.getInProgressRun()` (T015) in addition to its own store, refusing with the existing `execution_in_progress` shape.
- [ ] T040 [US3] Add the same cross-check in the other direction inside `POST /:id/execution/start` (`backend/src/api/externalCollections.ts`, T024/T034): check `execution/executionRunStore.ts`'s `getInProgressRun()` in addition to the uploaded store's own.
- [ ] T041 [US3] Add a static source label to each run list — `RunHistory` (the sub-component inside `frontend/src/components/ExecutionResultsPanel.tsx:392`, which only ever renders generated runs) gets a fixed "Generated" label/header, and `ExternalCollectionRunPanel.tsx`'s own run list (T030, which only ever renders `UploadedCollectionExecutionRun`s, always `source: "uploaded"`) gets a fixed "Uploaded" label/header. No shared field or component is needed — the two lists are already homogeneous by which panel they live in (research.md D9); do not add a `source` field to `ExecutionRun` to make this "shared" (see Phase 2 checkpoint note).
- [ ] T042 [P] [US3] Frontend RTL test covering the source label rendering (extend the existing run-history component's test file, or add one under `frontend/tests/unit/` if none exists) — asserts a generated run and an uploaded run in the same list are each labeled and visually distinguishable.
...
- [ ] T043 [P] Add the two-tab view switcher ("Guided Workflow" / "Import & Run Collection") to `frontend/src/App.tsx` (research.md D9, FR-011) — local component state only, no `react-router`; the "Import & Run Collection" tab renders `ExternalCollectionUpload`/`ExternalCollectionList`/`ExternalCollectionRunPanel` (T028-T030) with no dependency on `TestGenerationWorkflowPage`'s state.
- [ ] T044 [P] Frontend RTL test in `frontend/tests/unit/App.test.tsx` (extend or add): both tabs render, switching tabs preserves each view's own state, "Import & Run Collection" is reachable and functional with zero prior OpenAPI upload (FR-011).
- [ ] T045 [P] Integration test in `backend/tests/integration/externalCollections/scriptFidelity.test.ts` covering quickstart.md Scenario 7 (FR-008): a collection whose pre-request script computes a header the target server must actually receive, and whose test script depends on it — proves real script execution, not just literal request replay.
- [ ] T046 Run `npm test`, `npm run lint`, and `npm run build` from the repo root; fix any TypeScript/lint error introduced by this feature (CLAUDE.md §54/§55) before considering the feature complete.
- [ ] T047 Walk through quickstart.md's all 7 scenarios manually against a running local dev backend (`npm run dev -w backend` + frontend dev server) to validate the feature end-to-end, since RTL/Supertest do not exercise the real browser file-upload/confirmation-dialog flow.
