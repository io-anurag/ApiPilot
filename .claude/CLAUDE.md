# ApiPilot — Claude Code Instructions

> This file is the Claude Code counterpart of `.github/copilot-instructions.md`.
> The two documents carry the same engineering rules for different assistants; when a
> rule changes, update both so they do not drift.
...
Reference files and code locations as clickable paths, for example
`backend/src/testDesign/generateScenarios.ts:42`.

Search before writing: use the repository's specifications, `packages/shared-domain/`
contracts, and existing modules as context rather than inferring behavior from a file
name.
